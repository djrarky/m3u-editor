...
class VodRelationManager extends RelationManager
{
    ...
    protected function getBulkActionsWithParentPlaylist($ownerRecord): array
    {
        ...
        $changeParentPlaylistAction = Tables\Actions\BulkAction::make('change_parent_playlist')
        ...
        $insertIndex = collect($actions)->search(fn ($action) => $action->getName() === 'failover');
        if ($insertIndex === false) {
            $insertIndex = count($actions);
        }

        array_splice($actions, $insertIndex, 0, [$changeParentPlaylistAction]);

        $bulkActions[0]->actions($actions);
